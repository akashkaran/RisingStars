...
for index,row in df.iterrows():
    if((row['SR']>=sr_median)and(row['Avg']>=avg_median)and(row['R/I']>=rpi_median)and(row['HS']>=hs_median)and(row['50/I']>=fpi_median)and(row['6+4/BF']>=bpbf_median)):
...
df.to_csv('temptrainset.csv',mode = 'w')
